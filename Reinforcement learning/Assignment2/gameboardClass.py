...
class TGameBoard:
    ...
    def __init__(self,N_row,N_col,tile_size,max_tile_count,agent,stochastic_prob):
        ...
        # The fn_restart method resets the game board, tile count, and sets all spaces on the board to -1.
    ...
    def fn_restart(self):
        ...
        # The fn_new_tile method selects a random tile shape and sets the tile position to the center of the top row. 
        # If the maximum tile count has been reached, the gameover attribute is set to 1. 
        # Otherwise, the tile_count is incremented, and the agent is notified of the new state.
    ...
    def fn_new_tile(self):
        ...
        # The fn_check_boundary method takes in the current tile position and orientation 
        # and checks if the tile is within the boundaries of the game board.
    ...
    # checks if the new tile orientation is valid by comparing it to the length of the list of orientations for the current tile type.
    # If the new tile orientation is valid, the method saves the current tile position and orientation as old_tile_x and old_tile_orientation, respectively. 
    # It then updates the tile's position and orientation with the new values passed in as parameters.
    # The method then checks if the new tile position is valid using another method called fn_check_boundary(). 
    # If the new position is invalid, the method resets the tile's position and orientation to the previous values stored in old_tile_x and old_tile_orientation, respectively, and returns 1.
...
# This is the fn_drop() method which is responsible for dropping the current tile on the game board and checking for completed lines.
# It first finds the lowest position at which the current tile can be dropped by checking each column of the tile for the first occupied location on the game board. 
# It then updates the game board by changing the corresponding entries to 1.
# Next, it checks for completed lines and removes them, shifting the remaining rows down. 
# The number of removed lines is used to calculate the reward points.
# Finally, it calls fn_new_tile() to choose the next tile and returns the reward points.
